...
const assignment = "16 - Strings part 2";
...
function normaliseAuthorName(name) {
  const normalisedName = name.toLowerCase().replace("(contributor)", "").trim();
  const nameCapitalised = normalisedName
    .replace(normalisedName[0], normalisedName[0].toUpperCase())
    .replace(
      normalisedName[normalisedName.indexOf(" ") + 1],
      normalisedName[normalisedName.indexOf(" ") + 1].toUpperCase()
    );
  console.log(nameCapitalised);
}
...
function logBookTheme(bookTitle) {
  const normalisedTitle = bookTitle.toLowerCase();

  if (normalisedTitle.startsWith("computers")) {
    console.log("This book is about computers");
  } else if (
    normalisedTitle.includes("algorithms") &&
    normalisedTitle.includes("structures")
  ) {
    console.log("This book is about algorithms and data structures");
  } else if (
    normalisedTitle.endsWith("systems") ||
    (normalisedTitle.includes("system") &&
      !normalisedTitle.includes("operating"))
  ) {
    console.log(
...
logBookTheme("Computers of tomorrow");
logBookTheme("DSA: Data structures and algorithms");
logBookTheme("Designs concepts and systems");