...
console.log(
  (team1 < team2 && 'team1 is more likely to win') ||
    (team2 < team1 && 'team2 is more likely to win')
);
...
function calcAvgOdd(odds) {
  ...
  avgOdd = (oddSum / Object.keys(odds).length).toFixed(2);

  console.log(`the average odd is ${avgOdd}`);
}

calcAvgOdd(game.odds);
...
for (const [i, value] of Object.entries(game.odds)) {
  game[i]
    ? console.log(`Odd of victory ${game[i]}: ${value}`)
...
btn.addEventListener('click', function () {
  ...
  const wordsToProcess = inputText.split('\n');

  for (const [i, v] of wordsToProcess.entries()) {
    const normalizedInput = v.trim().toLowerCase();
    const capitalized2ndWord = normalizedInput.replace(
      /_([a-zA-Z])/g,
      normalizedInput[normalizedInput.indexOf('_') + 1].toUpperCase()
    );

    console.log(`${capitalized2ndWord.padEnd(20)}${'✅'.repeat(i + 1)}`);
  }
});
